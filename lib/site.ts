export const site = {
  name: 'Jordan Bangoji',
  tribeName: 'Tribe of Jordan',
  description: "Welcome to Jordan's Corner.",

  nav: [
    { href: '/', label: 'Home' },
    { href: '/#music', label: 'Music' },
    { href: '/#videos', label: 'Videos' },
    { href: '/shows', label: 'Shows' },
    { href: '/tribe', label: 'Tribe' },
    { href: '/contact', label: 'Contact' },
    { href: '/press', label: 'Press' },
  ],

  contactEmail: 'booking@yourdomain.com',

  socials: [
    { label: 'Spotify', href: 'https://open.spotify.com/artist/702zIZlmgDwWKAqWW4WX5d' },
    { label: 'Apple Music', href: 'https://music.apple.com/us/artist/jordan-bangoji/1529972930' },
    { label: 'YouTube', href: 'https://www.youtube.com/c/JordanBangoji' },
    { label: 'Instagram', href: 'https://www.instagram.com/1thjordan/' },
    { label: 'X', href: 'https://x.com/1thjordan' },
    { label: 'Facebook', href: 'https://www.facebook.com/1thjordan/' },
  ],
} as const
